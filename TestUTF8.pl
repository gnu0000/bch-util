...
sub Test
   {
   my $sample1 = "This is a test";
#   my $sample2 = "A \x{2264} b for some A";
   my $sample2 = "\x{8fd9}\x{662f}\x{4e00}\x{4e2a}\x{6d4b}\x{8bd5}";
   my $sample3 = FetchSample(3);

   my $s1_translated = Translate($sample1, "zh-CN");
   my $s1_transcoded = Transcode($s1_translated);
   my $s1_thru       = ThruDB($s1_transcoded, 1);
   my $s2_thru       = ThruDB($sample2, 2);

#   DumpSample($s1_translated,1);
...
#
   ...
   PrintRow($file, "sample1 translated", $s1_translated);
   PrintRow($file, "sample1 transcoded", $s1_transcoded);
   PrintRow($file, "sample1 thru db   ", $s1_thru      );
   PrintRow($file, "sample2           ", $sample2      );
   PrintRow($file, "sample2 thru db   ", $s2_thru      );
   PrintRow($file, "sample from DB    ", $sample3      );
   print $file "</table>\n";
...
sub ThruDB
   ...
   my $dest = FetchColumn("select text from test where id=$id");
   return $dest;
   }
